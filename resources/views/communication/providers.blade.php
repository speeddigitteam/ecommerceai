                    ...
                    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-[#161f2e] xl:sticky xl:top-24">
                        ...
                        <form method="POST" :action="editing ? '{{ url('/communication/providers/'.$channel) }}/' + editing.id : '{{ route('communication.providers.store', $channel) }}'">
                            ...
                            <div class="space-y-5 p-5">
                                ...
                                @else
                                    <fieldset class="space-y-3">
                                        <div class="flex items-center justify-between"><legend class="text-xs font-bold uppercase tracking-wider text-slate-400">API connection</legend><button type="button" @click="const f=$el.closest('form').elements; f.name.value='MRAM SMS'; f.url.value='https://msg.mram.com.bd/smsapi'; f.to_parameter.value='contacts'; f.message_parameter.value='msg'; f.api_key_parameter.value='api_key'; f.sender_parameter.value='senderid'; f.balance_url.value='https://msg.mram.com.bd/miscapi/{api_key}/getBalance'" class="rounded-lg bg-indigo-50 px-2.5 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-100 dark:bg-indigo-500/15 dark:text-indigo-300">Use MRAM preset</button></div>
                                        ...
                                        <label class="block text-sm font-semibold">Sender ID field<input name="sender_parameter" :value="editing?.settings?.sender_parameter || 'sender_id'" class="mt-2 w-full rounded-xl border-slate-300 text-sm dark:border-slate-700 dark:bg-slate-800"></label>
                                        <div class="grid grid-cols-2 gap-3"><label class="text-sm font-semibold">Message type<select name="message_type" class="mt-2 w-full rounded-xl border-slate-300 text-sm dark:border-slate-700 dark:bg-slate-800"><option value="auto">Auto detect</option><option value="text">Text</option><option value="unicode">Unicode</option></select></label><label class="text-sm font-semibold">Purpose<select name="label" class="mt-2 w-full rounded-xl border-slate-300 text-sm dark:border-slate-700 dark:bg-slate-800"><option value="transactional">Transactional</option><option value="promotional">Promotional</option></select></label></div>
                                        <label class="block text-sm font-semibold">Balance URL<input name="balance_url" type="url" :value="editing?.settings?.balance_url || ''" placeholder="https://provider.example/balance/{api_key}" class="mt-2 w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-800"><span class="mt-1 block text-xs font-normal text-slate-500">Use {api_key} where the encrypted key belongs.</span></label>
                                    </fieldset>
                                @endif
                            </div>
