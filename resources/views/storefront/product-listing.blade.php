<x-storefront-layout :title="$pageTitle" :description="$description" :canonical="$canonical">
    <main class="min-h-screen bg-[#f8fafc] py-8 dark:bg-[#111827] sm:py-12 lg:py-16">
        <div class="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8">
            <nav aria-label="Breadcrumb" class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                ...
                <span aria-current="page" class="text-slate-800 dark:text-slate-200">{{ $pageTitle }}</span>
            </nav>
            ...
            <header class="mt-6 flex flex-col gap-4 border-b border-slate-200 pb-7 dark:border-slate-800 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <p class="text-sm font-bold text-indigo-600 dark:text-indigo-400">{{ $eyebrow }}</p>
                    <h1 class="mt-1 text-3xl font-semibold tracking-tight text-slate-950 dark:text-white sm:text-4xl">{{ $pageTitle }}</h1>
                    <p class="mt-3 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-400">{{ $description }}</p>
                </div>
            ...
            @if ($products->isNotEmpty())
                <div class="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-5 lg:gap-5">
                    @foreach ($products as $product)
                        <div class="relative">
                            @if ($badge)
                                <span class="absolute left-3 z-10 rounded-full bg-indigo-600 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.12em] text-white shadow-sm {{ $product->sale_price && $product->price > $product->sale_price ? 'top-11' : 'top-3' }}">{{ $badge }}</span>
                            @endif
